...
const Carousel = () => {
  ...
  useEffect(() => {
    ...
    const normalizedStartIndex =
      ((bufferStartIndex % matches.length) + matches.length) % matches.length;
    ...
    setCurrentIndex((prevIndex) => {
      ...
      if (prevIndex === 0) {
        setBufferStartIndex((prev) => {
          const newIndex =
            (((prev - 1) % matches.length) + matches.length) % matches.length;
          return newIndex;
        });
      ...
      <div className="flex h-[400px] items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500">
            We couldn't find matches for you at the moment.
          </p>
        </div>
      </div>
    );
    ...
    <div className="relative mx-auto flex h-[600px] max-w-md flex-col items-center px-4">
              ...
              Array.from({ length: 5 }).map((_, idx) => (
                <div
                  ...
                  className={`h-1.5 w-6 rounded-full transition-all ${
                    idx ===
                    Math.floor(
                      ((bufferStartIndex + currentIndex) % matches.length) /
                        (matches.length / 5)
                    )
                      ? "bg-[#f83e67]"
                      : "bg-gray-200"
      ...
      {process.env.NODE_ENV === "development" && (
        <div className="absolute bottom-0 left-0 bg-black/70 text-white text-xs p-1 rounded">
          Match {((bufferStartIndex + currentIndex) % matches.length) + 1} of{" "}
          {matches.length}
        </div>
      )}
    </div>
